...
const SignupStudent = ({ navigation, route }) => {
  const emailUrl = route.params?.email || '';
  const classroomUrl = route.params?.classroom || '';
  ...
  useEffect(() => {
    setStudentInfo({ ...studentInfo, email: emailUrl, classroom: classroomUrl })
  }, [])
